...
When `--most-common-problem` is active, LogParser analyzes the parsed entries, picks the most common exception type, then the most common context within that type, then the most common message within that type/context pair, and emits a focused report for that highest-occurred problem. This applies to both text and JSON output; the structured JSON report emits a single `problem` object.
