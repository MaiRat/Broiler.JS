...
public partial class JSFloat32Array : JSTypedArray
{
    ...
    public override bool SetValue(uint index, JSValue value, JSValue receiver, bool throwError = true)
    {
        ...
        if (index >= length)
            return false;
        System.Array.Copy(BitConverter.GetBytes((float)number), 0, buffer.buffer, byteOffset + index * 4, 4);
        return true;
    }
    ...
    {
        var temp = new JSFloat32Array(TypedArrayParameters.From(in a, BYTES_PER_ELENENT));
