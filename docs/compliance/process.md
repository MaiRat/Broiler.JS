...
6. The pinned manifest files remain useful for focused local reruns and debugging via `python scripts/compliance/run_test262.py --path-file <manifest.txt>`, and `.github/workflows/test262-full-script-host.yml` remains available as a dedicated manual full-suite variant using the same sharded script-host mode, including a `shard_index` dispatch input that accepts one zero-based shard index or `-1` for all shards.
