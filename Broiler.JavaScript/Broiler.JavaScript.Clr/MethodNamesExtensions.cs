...
internal static class MethodNamesExtensions
{
    ...
    public static (T method, object[] values) Match<T>(this (T method, ParameterInfo[] parameters)[] methods, in Arguments a, in KeyString name)
    {
    ...
    public static (T method, ParameterInfo[] parameters)[] ToPairs<T>(this IEnumerable<T> methods)
        where T : MethodBase => [.. methods.Select(x => (method: x, parameters: x.GetParameters())).OrderByDescending(x => x.parameters.RequiredCount())];
